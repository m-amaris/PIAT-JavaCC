...
Por último, las producciones definen las reglas sintácticas que el analizador seguirá. A continuación se muestra una producción principal y una producción para manejar trazas del tipo MSA. Estas producciones se corresponden con la estructura mencionada anteriormente (\autoref{fig:arboltraza}):
...
% \textbf{\#TODO: AÑADIR TABLA ESTADOS LEXICOS}

El análisis comienza en el estado por defecto buscando el token ``@graph''. Una vez encontrado, se activa el estado GRAPH que busca el inicio de un recurso mediante el token ``@id''. Al encontrarlo, se activa el estado RESOURCE y se comienza la extracción de la información.
...
% \textbf{\#TODO: INSERTAR IMAGEN DE TIEMPOS DE EJECUCCION Y AÑADIR EXPLICACION}
...
%  \textbf{\#TODO: Añadir porcentajes, explicar porque es asi la grafica.}
...
% \textbf{[COMPLETAR]...en xpath se procesa muchas veces el fichero...xpath: procesa muchas veces el fichero. contar el numero de recursos a cada identificardor, varias expresiones xpath , que te va a obligar a ejecutar el fichero cada vez...
% }
...
Esta clase contiene un mapa para almacenar los datasets ---datasetsMap---%, una variable para contar el número de datasets (numDatasets), y 
,así como
...
% \textbf{\#TODO: hacer grafica en P5 en la que se vea que cuantas mas expresiones XPATH se hagan, más tarda en ejecutarse (ya que analiza varias veces el archivo. Hacer una segunda grafica en la que se diga antes que si aumenta el numero de información a analizar, ahí es donde xpath penca.
% Sin embargo, si se realise un pequeño evolutivo de la práctica en la que se pidiese extraer mas informacion, se notaria en gran medida la diferencia entre herramientas:}
