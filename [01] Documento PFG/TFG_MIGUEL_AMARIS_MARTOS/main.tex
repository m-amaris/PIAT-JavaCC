...
\usepackage{xcolor} % Mejoras sobre el paquete color
...
% Para poder poner los número en el código y que no se copien al seleccionar el código
\usepackage{accsupp} 
\newcommand{\noncopynumber}[1]{%
...
% Aquí se aplican las configuraciones globales de parskip y parindent
% \setlength{\parskip}{0pt} % definimos la separación entre párrafos
% \setlength{\parindent}{15pt} % definimos la identación de los párrafos
...
% definimos el estilo que queremos que tenga el código
\lstdefinestyle{java}{
    ...
    numbers=left, % Añade números de línea en el lado izquierdo
    numberstyle=\tiny\noncopynumber,      
    keepspaces=true,    
    language=Java,                 
    showspaces=false,                
...
\lstset{style=java} % configuramos el estilo creado por defecto
